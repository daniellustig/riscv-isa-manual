...
\item \label{ptechoice} Let $pte$ be the value of the PTE at address
...
\begin{commentary}\color{red}
  Since Sv39 and Sv48 both have PTE bits 63:54 reserved in the same way, I've tentatively specified their meanings in both to be identical.
\end{commentary}
...
Bits 62--60 indicate cacheability attributes associated with the virtual
address in question, as described in Section~\ref{virtualattributes}.

Bit 59 indicates the use of naturally aligned power-of-two (NAPOT)
address translation contiguity, as described in Section~\ref{napotpages}.

Bits 58--54 are reserved for future use.
...
%\hline
%0  & 0  & 0  & {\it reserved}                 \\
%0  & 0  & 1  & Force Cacheable                \\
%0  & 1  & 0  & Force Write-Combining          \\
%0  & 1  & 1  & Force Uncacheable              \\
%1  & 0  & 0  & {\it reserved}                 \\
%1  & 0  & 1  & Force Cacheable + Non-temporal \\
%1  & 1  & 0  & {\it reserved}                 \\
%1  & 1  & 1  & {\it reserved}                 \\
%\hline
%\end{tabular}
%\end{center}
%\caption{PTE Cacheability Attributes}
%\label{pteattrs}
%\end{table*}
%
%See Table~\ref{pteattrs}.
...
Bit 59 indicates that a PTE represents a translation that is part of a range of
contiguous virtual-to-physical translations with the same permissions at some
...
Table~\ref{ptenapot}.  Currently, NAPOT encodings are only supported for leaf
PTEs.  Depending on need, the NAPOT scheme may be extended to other
...
\end{tabular}
\end{center}
\caption{NAPOT Contigious Translation Encodings}
\label{ptenapot}
\end{table*}
...
The address translation process for Sv39 is identical to that of Sv32, except
that VALEN equals 39, LEVELS equals 3, PTESIZE equals 8, and the following two
steps in the virtual address translation process in Section~\ref{sv32algorithm}
are modified as follows:

\begin{enumerate}
  \setcounter{enumi}{\getrefnumber{ptechoice}}\addtocounter{enumi}{-1}
  \item
Let $pte$ be the value of the PTE at address
$a+j\times \textrm{PTESIZE}$ for any $0\leq j\leq \text{(PAGESIZE/PTESIZE)}$ such that $j[\textrm{PPNLEN} - 1:pte.napot\_bits] = vpn[i][\textrm{PPNLEN} - 1:pte.napot\_bits]$. (For Sv39, PTESIZE=8, PAGESIZE=$2^{12}$, and PPNLEN=9 for levels of the page table that support NAPOT PTEs.)
If accessing $pte$ violates a PMA or PMP check, raise an
access-fault exception corresponding to the original access type.

  \setcounter{enumi}{\getrefnumber{translationsuccess}}\addtocounter{enumi}{-1}
  \item The translation is successful. The translated physical address is
    given as follows:
  \begin{itemize}
  \item $\textit{pa.pgoff} = \textit{va.pgoff}$.
  \item If $i>0$, then this is a superpage translation and $pa.ppn[i-1:0]=va.vpn[i-1:0]$.
  \item $pa.ppn[i][\textrm{PPNLEN} - 1:pte.napot\_bits] = pte.ppn[i][\textrm{PPNLEN} - 1:pte.napot\_bits]$.
  \item If $pte.napot\_bits>0$, then this is a NAPOT translation, and $pa.ppn[i][pte.napot\_bits - 1:0] = va.vpn[i][pte.napot\_bits - 1:0]$.
  \item $pa.ppn[\textrm{LEVELS} - 1:i+1] = pte.ppn[\textrm{LEVELS} - 1:i+1]$.
  \end{itemize}
\end{enumerate}

\begin{commentary}
  Note that when bit 59 is equal to zero, indicating a non-NAPOT translation process, both steps reduce to their original form in Section~\ref{sv32algorithm}.
\end{commentary}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.6\textwidth]{figs/napot_pagewalk.pdf}
  \caption{An example NAPOT page table walk for Sv39}
  \label{sv39napotwalk}
\end{figure}

\begin{commentary}
  Figure~\ref{sv39napotwalk} shows how the NAPOT-aware translation algorithm
  represents an address translation request hitting on a previously-fetched
  NAPOT TLB entry, while avoiding referring directly to TLBs, which are
  microarchitectural rather than architectural state.  This scenario is
  represented by architecturally permitting the last level of the page table
  walk to directly access the page table entry corresponding to the original
  source of the NAPOT TLB entry (i.e., at offset $aj$), rather than accessing
  the PTE indexed by $vpn[0]$.

  In most real implementations, the page table walk itself will continue to
  follow the algorithm in Section~\ref{sv32algorithm} rather than choosing $j$
  arbitrarily during the walk.
\end{commentary}
...
\begin{commentary}
  ...
  it may not consult the PTE specified by the algorithm in
  Section~\ref{sv32algorithm} at all.  In particular, any A (Accessed) and/or D
  (Dirty) bit updates may be performed only on the NAPOT PTE, without updating
  the original PTE or any other NAPOT PTE(s) that may overlap the same memory
  range.  It is the responsibility of the operating system to account for this
  when NAPOT PTE(s) are in use.
...
Section~\ref{napotpages}, except VALEN equals 48, LEVELS equals 4, and
PTESIZE equals 8.
