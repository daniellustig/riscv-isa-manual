...
\begin{commentary}
  ...
  This allows a region to be covered with one PTE instead of multiple, which in
  turn can help relieve TLB pressure in some scenarios.  The NAPOT encoding is
...
Figure~\ref{sv39napotwalk} shows an example page table walk for an Sv39 NAPOT
translation process.  The last level of the page table walk uses a 64KiB
NAPOT PTE rather than the PTE indexed by $vpn[0]$.  The physical address
ultimately generated therefore pulls the high-order bits of $ppn[0]$ from the
$ppn[0]$ field of the PTE itself and the low-order bits of $ppn[0]$ from
$vpn[0]$.
