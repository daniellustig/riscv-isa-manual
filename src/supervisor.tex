...
between page-table updates and subsequent address translations.  In other
words, it does not imply any invalidation of address-translation caches.
...
It does this by flushing the {\em address-translation cache(s)} associated with
a hart.  Further details on the behavior of this instruction are
...
address translation.  SFENCE.VMA
...
\begin{itemize}
\item If {\em rs1}={\tt x0} and {\em rs2}={\tt x0}, the fence invalidates
      all address-translation cache entries, for all address spaces.  In
      addition, all in-flight translations are terminated.
\item If {\em rs1}={\tt x0} and {\em rs2}$\neq${\tt x0}, the fence invalidates
      all address-translation cache entries holding page table entries that
      are non-global and that match the address space identified by integer
      register {\em rs2}.  In addition, all in-flight translations being
      performed for the address space identified by integer register {\em rs2}
      are terminated.
\item If {\em rs1}$\neq${\tt x0} and {\em rs2}={\tt x0}, the fence invalidates
      all address-translation cache entries holding leaf page table entries
      corresponding to the virtual address in {\em rs1}, for all address
      spaces.
\item If {\em rs1}$\neq${\tt x0} and {\em rs2}$\neq${\tt x0}, the fence
      invalidates all address-translation cache entries holding non-global
      leaf page table entries corresponding to the virtual address in {\em
      rs1}, and matching the address space identified by integer register {\em
      rs2}.
\end{itemize}
...
speculatively.  The results of these reads may be held in an incoherent {\em
address-translation cache} but not shared with other harts.  The
address-translation cache may hold an arbitrary number of entries, including
an arbitrary number of entries for the same address and ASID.
Address-translation cache entries may only be established for
...
It is permitted for multiple address-translation cache entries to co-exist for the
same address.  This represents the fact that
...
interrupted by other S-mode writes to the page table, as such interruptions
could lead to A/D bits being set on PTEs that are no longer valid.  Simple
implementations may instead generate page-fault exceptions.
...
\setcounter{enumi}{-1}
\item Let $satp$ be the value of the {\tt satp} register.

\item Let $a$ be $\text{\em satp}.ppn \times \textrm{PAGESIZE}$, and let $i=\textrm{LEVELS} - 1$. (For Sv32, PAGESIZE=$2^{12}$ and LEVELS=2.)

\item Let $b=a+va.vpn[i]\times \textrm{PTESIZE}$.  Then either:
  \begin{itemize}
    \item Let $pte$ take the value of any address-translation cache entry for
      address $b$ such that either the address space identifier of the entry
      matches the address space identified by $\text{\em satp}.ASID$, or $pte.g=1$.
    \item Let $pte$ be the value of the PTE at address $b$.  If accessing $pte$
        violates a PMA or PMP check, raise an access-fault exception
        corresponding to the original access type.  Otherwise, create an
        address-translation cache entry mapping $b$ to the value $pte$ for
        the address space identified in $\text{\em satp}.ASID$.  The load of
        PTE and the creation of the address-translation cache entry must be
        atomic; in particular, no intervening SFENCE may be perceived to have
        occurred in between.
  \end{itemize}
  (For Sv32, PTESIZE=4.)
...
\item If $pte.a=0$, or if the original memory access is a store and $pte.d=0$, either
  ...
  \begin{itemize}
  \item If $pte$ was loaded from the address-translation cache in step 2, go
    back to step 2 and load $pte$ from address $b$ instead.
  ...
  \item Set $pte.a$ to 1 and, if the original memory access is a store, also set
    $pte.d$ to 1.  Create a new address-translation cache entry mapping $b$ to
    the value $pte$ for the address space identified in $\text{\em satp}.ASID$.
  \item This update, the loading of $pte$ in step 2, and the creation of the
    address-translation cache entry must be atomic; in particular, neither an
    intervening store to the PTE nor an SFENCE may be perceived to have
    occurred in-between.
  \end{itemize}
...
\begin{commentary}
  The translation algorithm explicitly permits an arbitrary number of
  translations to begin and then pause at any point for some arbitrary amount
  of time before continuing.  This has two notable implications: 1) at any
  time, the entire page table (i.e., all entries reachable by executing the
  algorithm) may be loaded into the address-translation cache, and 2) switching
  {\tt satp} does not terminate in-flight translations.  An implementation's
  page table walker may proceed with any {\tt satp} value that was previously
  valid.
\end{commentary}
...
Bit 63 indicates that the PTE uses a custom implementation-specific encoding.
If bit 63 is set, the algorithm for virtual-to-physical address translation
is implementation-defined.  If bit 63 is not set,
the algorithm for virtual-to-physical address translation is described in
Section~\ref{napotpages}.

Bit 62 indicates the use of naturally aligned power-of-two (NAPOT)
address translation contiguity, as described in Section~\ref{napotpages}.

Bits 61--\textcolor{red}{xx} indicate cacheability attributes associated with the virtual
address in question, as described in Section~\ref{virtualattributes}.

Bits \textcolor{red}{xx}--54 are reserved for future use.
...
\subsection{Naturally Aligned Power-of-Two Translation Contiguity}
\label{napotpages}

Bit 62 indicates that a PTE represents a translation that is part of a range of
contiguous virtual-to-physical translations with the same values for PTE bits
5--0.
naturally aligned power-of-2 (NAPOT) granularity larger than the base page
size.

\begin{commentary}
  The motivation for a NAPOT PTE is that it can be cached in a TLB as a single
  entry representing the contiguous region as if it were a single (large) page.
  This can help relieve TLB pressure in some scenarios.  The NAPOT encoding is
  designed to fit within the pre-existing Sv39 and Sv48 address translation
  algorithms so as not to disrupt existing implementations or designs that
  choose not to implement the scheme.
  ...
  for supervisor control.  The D and A bits may not be identical even in
  typical use cases, as described below.  However, there may be short periods
  when supervisor software is updating the page table where the PTEs within a
  NAPOT range are inconsistent.  In this scenario, it is up to supervisor
  software to insert SFENCE instructions as appropriate.  If the PTEs within a
  NAPOT range are inconsistent for this reason or any other, the behavior
  described in Section~\ref{sec:sfence.vma} still holds: it is unpredictable
  which PTE(s) will be used, but the behavior is otherwise well defined.
\end{commentary}

\begin{table*}[h!]
\begin{center}
\begin{tabular}{|c|c||c|l|}
%\multicolumn{4}{c}{Option 1: NANAPOT: an encoding that more naturally extends} \\
%\multicolumn{4}{c}{to other intermediate NAPOT sizes} \\
\hline
Bit 59 & $pte.ppn[i]$      & $pte.napot\_bits$ & Meaning if $i=0$         \\
\hline
0      & {\tt y~yyyy~yyyy} & 0                 &  Non-NAPOT PTE           \\
1      & {\tt y~yyyy~1zzz} & 4                 &  64KB contiguous region  \\
%1      & {\tt y~yyyy~yy1z} & 2                 &  16KB contiguous region \textcolor{red}{(tentative)} \\
%1      & {\tt y~yyyy~1z0z} & 4                 &  64KB contiguous region  \\
%1      & {\tt y~yy1z~0z0z} & 6                 &  256KB contiguous region \textcolor{red}{(tentative)} \\
\hline
%\multicolumn{4}{c}{} \\
%\multicolumn{4}{c}{Option 2: an encoding that uses zeros for 64KB pages, allowing} \\
%\multicolumn{4}{c}{for more flexibility for future non-NAPOT uses of the `{\tt z}' bits} \\
%\hline
%Bit 59 & $pte.ppn[i]$      & $pte.napot\_bits$ & Meaning if $i=0$         \\
%\hline
%0      & {\tt y~yyyy~yyyy} & 0                 &  Non-NAPOT PTE           \\
%1      & {\tt y~yyyy~yy1z} & 2                 &  16KB contiguous region \textcolor{red}{(tentative)} \\
%1      & {\tt y~yyyy~z00z} & 4                 &  64KB contiguous region  \\
%1      & {\tt y~yyzz~z10z} & 6                 &  256KB contiguous region \textcolor{red}{(tentative)} \\
%\hline
\multicolumn{4}{l}{{\tt y}: subset of PPN used for address translation} \\
\multicolumn{4}{l}{{\tt z}: reserved for future use; must be set to zero} \\
\end{tabular}
\end{center}
...
The list of currently supported NAPOT PTE encodings is shown in
Table~\ref{ptenapot}.  Currently, NAPOT encodings are only supported for 4KiB
leaf PTEs.
...
%\begin{commentary}\color{red}
%  TODO: choose an option in Table~\ref{ptenapot}.
%\end{commentary}

NAPOT PTEs follow the same address translation algorithm in
Section~\ref{sv32algorithm}, except that the following step is added to the end
of the algorithm:
\begin{itemize}
    \item Create address-translation cache entries mapping
      $a + va.vpn[j] \times \textrm{PTESIZE}$ to $pte$ for the address space
      identified in ${\tt satp}.ASID$, for all $j$ such that
      $j[9:napot\_bits]=i[9:napot\_bits]$.  The creation of these entries, the
      loading of $pte$ in step 2, and (if applicable) the update of the PTE's
      A and D bits must be atomic; in particular, no SFENCE may be perceived to
...
\begin{commentary}
  ...
  it might not consult the PTE specified by the algorithm in
  Section~\ref{sv32algorithm} at all.  The operating system must query all
  ...
  it is recommended that the OS also set the A and/or D bits for other NAPOT
  aliases as appropriate in order to avoid unnecessary traps.

  It is also the responsibility of the OS and/or hypervisor to configure the
  page table in a way that there are no inconsistencies between NAPOT PTEs and
  non-NAPOT PTEs.  If an update needs to be made, the OS generally should first
  mark all of the PTEs invalid, then issue an SFENCE.VMA, then update the
  PTE(s), as described in Section~\ref{sec:sfence.vma}.  If there are any
  inconsistencies, then the effect is the same as when SFENCE.VMA is used
  incorrectly: one of the translations will be chosen, but the choice is
  unpredictable.
\end{commentary}

\subsubsection{NAPOT Page Table Entry Litmus Tests}
\begin{commentary}\color{red}
  We might want to move this to an appendix or something by the time we're
  done.  I'm not worried about that now.  For now I'd rather just keep this all
  in one place.

  If anybody has more tests or examples to add here, please let me know and/or
  create a PR.
\end{commentary}

\paragraph{SFENCE Terminating In-Progress Translations}
\begin{figure}[h!]
\begin{center}
\begin{verbatim}
...
\end{verbatim}
\end{center}
\caption{Sample code showing that SFENCE must terminate in-progress translations}
\label{pte_sfence}
\end{figure}

Figure~\ref{pte_sfence} shows that SFENCE instructions with
{\em rs1}=0 must terminate in-progress translations.  Otherwise, any
translations that had proceeded only as far as reading {\tt satp} might
continue to use the old value of {\tt satp}.PPN, which is no longer valid and
might even now hold some unrelated data.  The same reasoning also applies at
subsequent levels of the page table.

SFENCE instructions with {\em rs1}$\neq$0 do not need to terminate in-progress
translations because the access of the leaf PTE or address-translation cache
entry and the potential creation of a new address-translation cache entry are
required to be atomic with respect to any SFENCE instructions.

\paragraph{Changing Modes During a Translation}
\begin{figure}[h!]
\begin{center}
\begin{verbatim}
  ...
  <The previously-launched still-in-flight translation reads that updated PTE>
  <M-mode updates that same PTE again, to some other value>
  csrw satp, <prior value of satp>
\end{verbatim}
\end{center}
\caption{Sample code for the behavior of in-flight translations}
\label{pte_inflight}
\end{figure}

Figure~\ref{pte_inflight} shows that in-flight address translations (page table
walks) are not terminated during a switch to {\tt satp}.  At the end of this
contrived example, it is legal for the address-translation cache to store any
or all of the three values for the PTE at {\tt a0}: the original value, the
intermediate value, and the final value.  The middle choice is not consistent
with executing the translation algorithm only while {\tt satp} is valid and
has not changed, but it is consistent with a translation process that started
while {\tt satp} was valid and continued even after {\tt satp} was switched.

\paragraph{A/D Bit Updates}
\begin{figure}[h!]
\begin{center}
\begin{verbatim}
  ...
  <The HW page table walker sets D=1 to the no-longer-valid PTE>
  <The OS swaps the page out without writing it back, since it thinks D=0>
\end{verbatim}
\end{center}
\caption{A/D bit updates happening concurrently with OS PTE updates}
\label{nonatomic_pte_ad}
\end{figure}

Figure~\ref{nonatomic_pte_ad} shows the consequence of allowing steps 2-7 in
the translation algorithm to be performed non-atomically.  If the OS updates
a page that is also concurrently being updated by a hardware page table walker,
and steps 2-7 are performed non-atomically, then it is possible the walker will
write D=1 to a PTE that is no longer valid.  This could lead to the OS not
realizing the page was written, which in turn could lead to data loss if the
page is evicted from memory.  It could also clobber data written by the OS
to the A/D bit positions in the invalid PTE's memory.  The atomicity
requirement ensures that such situations will be avoided.
...
\end{verbatim}
\end{center}
\caption{Sample code for NAPOT A/D bit updates}
\label{napot_pte_ad}
\end{figure}

Figure~\ref{napot_pte_ad} shows that NAPOT PTEs can create
address-translation cache entries with accessed and/or dirty bits set to 1.
As the translation algorithm in Section~\ref{sv32algorithm} specifies, A
and/or D bit updates will only be performed in memory, and only on the PTE
identified by the algorithm.  In other words, even though both PTEs are NAPOT,
a final outcome of {\tt PTE(a0).d == 1}, {\tt PTE(a1).d == 0} is not legal,
because it is not consistent with executing the algorithm as specified.
...
therefore a final outcome with {\tt PTE(a0).d == 0} is legal.  The translation
algorithm executed for the final store might hit on one of those
address-translation cache entries, and therefore will not need to update the
PTE for {\tt a0} in memory.

\subsection{Virtual Address Attributes}
\label{virtualattributes}
