...
\begin{commentary}
SFENCE.VMA is used by higher privilege levels to synchronize page table
...
\item If {\em rs1}={\tt x0} and {\em rs2}$\neq${\tt x0}, the fence invalidates
      all address-translation cache entries pointing to page table entries that
      are non-global and that match the address space identified by integer
      register {\em rs2}.
\item If {\em rs1}$\neq${\tt x0} and {\em rs2}={\tt x0}, the fence invalidates
      all address-translation cache entries pointing to leaf page table entries
      corresponding to the virtual address in {\em rs1}, for all address
      spaces.
\item If {\em rs1}$\neq${\tt x0} and {\em rs2}$\neq${\tt x0}, the fence
      invalidates all address-translation cache entries pointing to non-global
...
\item Let $b=a+va.vpn[i]\times \textrm{PTESIZE}$.  Either:
  \begin{itemize}
    \item Let $pte$ take the value of any address-translation cache entry for
      ...
      matches the address space identified by ${\tt satp}.ASID$, or $pte.g=1$.
    \item Let $pte$ be the value of the PTE at address $b$.  If accessing $pte$
      ...
      address-translation cache entry mapping $b$ to the value $pte$ for the
      address space identified in ${\tt satp}.ASID$.
  \end{itemize}
  (For Sv32, PTESIZE=4.)
...
\begin{commentary}
  This added step represents the creation of a single TLB entry covering the
  entire NAPOT region.
\end{commentary}
