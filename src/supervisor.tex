...
\item Let $b=a+va.vpn[i]\times \textrm{PTESIZE}$.  Then either:
  \begin{itemize}
    ...
    \item Let $pte$ be the value of the PTE at address $b$, accessed with S-mode
      permissions.  If accessing $pte$ violates a PMA or PMP check, raise an
      access-fault exception corresponding to the original access type.
      Otherwise, create an address-translation cache entry mapping $b$ to the
      value $pte$ for the address space identified in $\text{\em satp}.ASID$.
  \end{itemize}
  (For Sv32, PTESIZE=4.)
...
\item If $pte.a=0$, or if the memory access is a store and $pte.d=0$, either
  ...
  \begin{itemize}
  \item If an S-mode store to $pte$ would violate a PMA or PMP check, raise an
    access-fault exception corresponding to the original access type.
  ...
  \item This update and the loading of $pte$ in step 2 must be atomic; in
    ...
    occurred in-between.  $pte$ must also have been loaded from address $b$ in
    the first place; A and/or D bit updates may not be performed directly on
...
\begin{commentary}
  ...
  translations to proceed to step 3 and then pause for some arbitrary amount of
  time before continuing.  This has two notable implications: 1) at any time,
  the entire page table (i.e., all entries reachable by executing the algorithm)
  may be loaded into the address-translation cache, and 2) switching {\tt satp}
  does not terminate in-flight translations.  An implementation's page table
  walker may proceed with any {\tt satp} value that was previously valid.
\end{commentary}
...
Bits 61--xx indicate cacheability attributes associated with the virtual
...
leaf PTEs.  Depending on need, the NAPOT scheme may be extended to other
intermediate page sizes and/or to other levels of the page table in the future.
...
\begin{itemize}
    \item Create address-translation cache entries mapping
      ...
      $j[9:napot\_bits]=i[9:napot\_bits]$.
\end{itemize}
...
\begin{commentary}
  ...
  (even if the PTEs for the other 4KiB regions differ).
\end{commentary}
...
\begin{commentary}
  ...
  it is recommended that the OS also set the A and/or D bits for all NAPOT
  aliases as well in order to avoid unnecessary traps.
