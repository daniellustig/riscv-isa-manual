...
subsequent instructions in that hart to the memory-management data
structures.  It is also used to invalidate entries in the {\em
address-translation cache(s)} (See Section~\ref{sv32algorithm}) associated with
a hart.
...
\item If {\em rs1}={\tt x0} and {\em rs2}={\tt x0}, the fence orders all
      ...
      address spaces, before all implicit accesses by subsequent instructions
      in the hart to the memory-management data structures.  It also invalidates
      all address-translation cache entries, for all address spaces.
\item If {\em rs1}={\tt x0} and {\em rs2}$\neq${\tt x0}, the fence orders
      all reads and writes made to any level of the page tables before all
      implicit accesses by subsequent instructions in the hart to the
      memory-management data structures, but only for the address space
      identified by integer register {\em rs2}.
      Accesses to {\em global} mappings (see Section~\ref{sec:translation})
      are not ordered.  The fence also invalidates all address-translation cache
      entries matching the address space identified by integer register
      {\em rs2}, except for entries containing global mappings.
\item If {\em rs1}$\neq${\tt x0} and {\em rs2}={\tt x0}, the fence orders
      only reads and writes made to leaf page table entries corresponding to the
      virtual address in {\em rs1} before implicit reads by subsequent
      instructions in the hart to those entries, for all address spaces.
      It also invalidates all address-translation cache entries that contain
      leaf page table entries corresponding to the virtual address in {\em rs1},
      for all address spaces.
\item If {\em rs1}$\neq${\tt x0} and {\em rs2}$\neq${\tt x0}, the fence
      orders only reads and writes made to leaf page table entries
      corresponding to the virtual address in {\em rs1} before implicit reads
      by subsequent instructions in the hart to those entries, for the address
      space identified by integer register {\em rs2}.
      Accesses to global mappings are not ordered.  The fence also invalidates
      all address-translation cache entries that contain leaf page table
      entries corresponding to the virtual address in {\em rs1} and that
      match the address space identified by integer register {\em rs2},
...
Entries in the address-translaton cache may then satisfy subsequent step 2
...
translations.  The address-translation cache cannot be used for step 7;
accessed and dirty bits may only be updated in memory directly.
...
\begin{commentary}
  ...
  design, it is possible for multiple entries to match a single address if, for
  ...
  rs1}={\tt x0}.  In this case, just as if an SFENCE.VMA is not executed between
...
valid.  Such speculative executions have the effect of pre-populating the
address-translation cache.  Speculative executions of the address-translation
algorithm behave as non-speculative executions of the algorithm do, except that
they cannot set the dirty bit for a PTE, they cannot trigger an exception,
and they cannot create address-translation cache entries if those entries
would have been invalidated by any SFENCE.VMA instruction executed by the hart
since the speculative execution of the algorithm began.
...
\begin{commentary}
  A consequence of implementations being permitted read the translation data
  structures arbitrarily early and speculatively is that at any time, all
  all page table entries reachable by executing the algorithm may be loaded into
...
\begin{commentary}
  ...
  more entries representing the contiguous region as if it were a single (large)
  page covered by a single translation.  This compaction can help relieve
  TLB pressure in some scenarios.  The NAPOT encoding is designed to fit within
  the pre-existing Sv39 and Sv48 address translation algorithms so as not to
  disrupt existing implementations or designs that choose not to implement the
  scheme.
  ...
  for supervisor software control.  The D and A bits may not be identical even
  in typical use cases, as described below.  However, there may be short periods
  ...
  software to insert SFENCE.VMA instructions as appropriate.  If the PTEs within a
...
1      & {\tt y~yyyy~1zzz} & 4                 &  64KiB contiguous region \\
\hline
...
Section~\ref{sv32algorithm}, except that during step 2, implicit reads of NAPOT
page table entries may create address-translation cache entries mapping $a +
va.vpn[j] \times \textrm{PTESIZE}$ to $pte$ for the address space identified in
{\em satp} as loaded by step 0, for all $j$ such that
...
\section{Virtual Memory Litmus Tests}
\begin{commentary}\color{red}
...
\paragraph{SFENCE.VMA Terminating In-Progress Translations}
...
The test above shows that SFENCE.VMA instructions with {\em rs1}={\tt x0}
must order speculative executions of the address-translation algorithm.
Otherwise, any translations that had proceeded only as far as reading {\tt satp}
might continue to use the old value of {\tt satp}.PPN, which is no longer valid
...
not terminated during a switch to {\tt satp}.  At the end of this contrived
example, it is legal for the address-translation cache to store any or all of
the three values for the PTE at {\tt a0}: the original value, the intermediate
value, and the final value.  The middle choice is not consistent with executing
the translation algorithm only while {\tt satp} is valid and has not changed,
but it is consistent with a translation process that started while {\tt satp}
was valid and continued even after {\tt satp} was switched.
...
with accessed and/or dirty bits set to 1.  As the translation algorithm in
Section~\ref{sv32algorithm} specifies, A and/or D bit updates will only be
performed in memory, and only on the PTE identified by the algorithm.  In other
words, even though both PTEs are NAPOT, a final outcome of {\em d}=1 for the PTE
at {\tt a0} and {\em d}=0 for the PTE at {\tt a1} is not legal, because it is
not consistent with executing the algorithm as specified.
...
therefore a final outcome with {\em d}=0 for the PTE at {\tt a0} is legal.  The
